    ...
    x %>% 
        ...
        lapply(function(y){
            if(str_detect(y$formula, "Segmented")){
                ...
                lm(formula = y$formula, data = div)
            }
        })
}

models_fit_separated <- function(x){
    x %>% 
        split(1:nrow(.)) %>%
        lapply(function(y){
            df <- filter(div, year == y$year, tur == y$tur)
            if(str_detect(y$formula, "Segmented")){
                ###
                set.seed(1); df <- df %>% 
                    mutate_if(is.numeric, ~`+`(.x, rnorm(nrow(df), sd = 1e-5)))
                segmented::segmented(lm(
                    str_replace_all(y$formula, "Segmented", ""), 
                    data = df), seg.Z = ~km)
                ###
...
models_pred_separated <- function(fits){
    # mclapply(
    lapply(
        fits, 
        function(x){
            c(if("segmented" %in% class(x)){x$psi[2]}else{numeric()}, seq(1, 32, by = 0.5)) %>% 
                ...
                transmute(km, predicted = predict(x, .)) 
        }
        # mc.cores = detectCores()-1
    )
}
...
# all in one 
{
models_allin1 <- expand_grid(
        resp = c("abu", "abuLog", "nsp", "nsp100", "shan"),
        formula = c(
            "km + year + tur", "kmSegmented + year + tur", 
            "km + km2 + year + tur", "kmLog + year + tur")
    ) %>% 
    mutate(
        formula = paste0(resp, " ~ ", formula)) %>% 
    mutate(fits = models_fit_allin1(.), 
...
models_separated <- expand_grid(
    ...
    mutate(
        formula = paste0(resp, " ~ ", formula)) %>% 
    mutate(fits = models_fit_separated(.), 
        pred = models_pred_separated(fits),
        aic = map_dbl(fits, ~AIC(.x)), 
        r2 = map_dbl(fits, ~summary(.x)$adj.r.squared)
    ) %>% 
    mutate(aic2 = map_dbl(fits, ~manual_aic(.x, df_observed = 3)), 
...
df1 %>% 
    ggplot(aes(x = km, y = predicted, fill = tur, color = tur, shape = tur)) +
    # geom_point(size = 3, data = df2, alpha = 0.5, color = "black") +
    geom_line() +
    facet_grid(cols = vars(year), rows = vars(resp), scales = "free") +
